...
def findFaces(colored_img, scalefactor = 1.1, neighbors = 5):
  ...
  #just making a copy of image passed, so that passed image is not changed 
    ...
    frontface_cascade = cv2.CascadeClassifier('data/haarcascade_frontalface_alt.xml')
    
    #get side face trainig set
    ...
    faces = frontface_cascade.detectMultiScale(gray, scaleFactor=scalefactor, minNeighbors=neighbors);
    sidefaces  = sideface_cascade.detectMultiScale(gray, scaleFactor=scalefactor, minNeighbors=neighbors);
    
    #join the side and front faces
    #faces = numpy.vstack((frontfaces, sidefaces))
    print (len(faces), len(sidefaces))
